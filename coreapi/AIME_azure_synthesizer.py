...
class AzureSynthesizer(BaseSynthesizer[AzureSynthesizerConfig]):
    ...
    async def send_chunk_to_inference(self, chunk, sr, characterID):
        response = requests.post(url="http://{}:{}/http_inference?f0_up_key=0&f0_method=harvest&index_rate=0.4&sr={}&character_id={}".format(INFERENCE_HOST, INFERENCE_PORT, sr, characterID),
    ...
    async def create_speech(
        ...
        async def get(audio_data_stream):
            data = b''
            while True:
                audio_buffer = bytes(chunk_size)
                chunk = audio_data_stream.read_data(audio_buffer)
                if not chunk:
                    break
                data += audio_buffer
            return data
        ...
        data = await get(audio_data_stream)
        rs = await self.send_chunk_to_inference(data, self.synthesizer_config.sampling_rate, characterID)

        # print(self.synthesizer_config.sampling_rate)
        output_bytes_io = io.BytesIO()
        in_memory_wav = wave.open(output_bytes_io, "wb")
        in_memory_wav.setnchannels(1)
        in_memory_wav.setsampwidth(2)
        in_memory_wav.setframerate(self.synthesizer_config.sampling_rate)
        in_memory_wav.writeframes(rs)
